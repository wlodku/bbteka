  ...
  class UsersController < BaseController
    ...
    def upload
      ...
      when '.csv'
        ...
        keys = %w(lp name surname login grade)
    ...
    def import_create
      ...
      params['users'].each do |user|
        next unless user['add'] == '1'
        u = User.new(name: user['name'], surname: user['surname'], login: user['login'], grade_id: user['grade_id'], email: Faker::Internet.email + (1..100_000).to_a.sample.to_s, password: 'mamamama', password_confirmation: 'mamamama')
        if u.valid?
          users << u
        else
          @new << User.new
